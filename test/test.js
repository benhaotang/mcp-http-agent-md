...
function parseResponse(text) {
  ...
  // Try to decode as toon format first
  try {
    ...
    try {
      ...
      // If both fail, return raw text wrapped in object
      return { _raw: text };
    }
  }
}
...
async function run() {
  ...
  try {
    ...
    const badPayload = parseResponse(badWrite.content?.[0]?.text || '{}');
    ...
    const badStatePayload = parseResponse(badState.content?.[0]?.text || '{}');
    ...
    const vInit = parseResponse(vInitRes.content?.[0]?.text || '{}');
    ...
    const vLogs1 = parseResponse(vLogs1Res.content?.[0]?.text || '{}');
    ...
    const vWrite = parseResponse(vWriteRes.content?.[0]?.text || '{}');
    assert(vWrite.hash && typeof vWrite.hash === 'string', 'write_agent should return hash');
    const vLogs2 = parseResponse((await client.callTool({ name: 'list_project_logs', arguments: { project_id: vprojId } })).content?.[0]?.text || '{}');
    ...
    const vAdd = parseResponse(vAddRes.content?.[0]?.text || '{}');
    assert(vAdd.hash && typeof vAdd.hash === 'string', 'progress_add should return hash when adding new tasks');
    const vLogs3 = parseResponse((await client.callTool({ name: 'list_project_logs', arguments: { project_id: vprojId } })).content?.[0]?.text || '{}');
    ...
    const vSet = parseResponse(vSetRes.content?.[0]?.text || '{}');
    assert(vSet.hash && typeof vSet.hash === 'string', 'progress_set_new_state should return hash when changes occur');
    const vLogs4 = parseResponse((await client.callTool({ name: 'list_project_logs', arguments: { project_id: vprojId } })).content?.[0]?.text || '{}');
    ...
    const vRename = parseResponse(vRenameRes.content?.[0]?.text || '{}');
    assert(vRename.hash && typeof vRename.hash === 'string', 'rename_project should return hash');
    const vLogs5 = parseResponse((await client.callTool({ name: 'list_project_logs', arguments: { project_id: vprojId } })).content?.[0]?.text || '{}');
    ...
    const rev = parseResponse(revRes.content?.[0]?.text || '{}');
    assert(rev.hash === initHash, 'revert_project should return reverted hash');
    const vLogsAfterRevert = parseResponse((await client.callTool({ name: 'list_project_logs', arguments: { project_id: vprojId } })).content?.[0]?.text || '{}');
    ...
    const tasksAfterRevert = parseResponse((await client.callTool({ name: 'read_progress', arguments: { project_id: vprojId } })).content?.[0]?.text || '{}');
    ...
    const initPayload = parseResponse(init.content?.[0]?.text || '{}');
    ...
    const addTasks = parseResponse(addTasksRes.content?.[0]?.text || '{}');
    ...
    const noMatchPayload = parseResponse(noMatch.content?.[0]?.text || '{}');
    ...
    let tasksPayload = parseResponse(readProgress.content?.[0]?.text || '{}');
    ...
    const onlyPendingJson = parseResponse(onlyPending.content?.[0]?.text || '{}');
    ...
    const onlyDoingJson = parseResponse(onlyDoing.content?.[0]?.text || '{}');
    ...
    const onlyDoneJson = parseResponse(onlyDone.content?.[0]?.text || '{}');
    ...
    const dup = parseResponse(dupRes.content?.[0]?.text || '{}');
    ...
    const setPayload = parseResponse(setRes.content?.[0]?.text || '{}');
    ...
    const init3Payload = parseResponse(init3Res.content?.[0]?.text || '{}');
    ...
    const wrPayload = parseResponse(wr.content?.[0]?.text || '{}');
    ...
    const pt3 = parseResponse(rp3.content?.[0]?.text || '{}');
    ...
    const upd = parseResponse(updRes.content?.[0]?.text || '{}');
    ...
    const p4 = parseResponse(rp4.content?.[0]?.text || '{}');
    ...
    const p5 = parseResponse(rp5.content?.[0]?.text || '{}');
    ...
    const p5a = parseResponse(rp5arch.content?.[0]?.text || '{}');
    ...
    const init4Payload = parseResponse(init4Res.content?.[0]?.text || '{}');
    ...
    const p6 = parseResponse(rp6.content?.[0]?.text || '{}');
    ...
    const forbidEdit = parseResponse(forbidEditRes.content?.[0]?.text || '{}');
    ...
    const forbidUnlockChild = parseResponse(forbidUnlockChildRes.content?.[0]?.text || '{}');
    ...
    const p7 = parseResponse(rp7.content?.[0]?.text || '{}');
    ...
    const childEdit = parseResponse(childEditRes.content?.[0]?.text || '{}');
    ...
    const p8 = parseResponse(rp8.content?.[0]?.text || '{}');
    ...
    const gen = parseResponse(genRes.content?.[0]?.text || '{}');
    ...
    const spInitProj = parseResponse(spInitProjRes.content?.[0]?.text || '{}');
    ...
    const spInitPayload = parseResponse(spInit.content?.[0]?.text || '{}');
    ...
    const spRev1 = parseResponse(spReview1.content?.[0]?.text || '{}');
    ...
    const spUpdPayload = parseResponse(spUpd.content?.[0]?.text || '{}');
    ...
    const spRev2 = parseResponse(spReview2.content?.[0]?.text || '{}');
    ...
    const spRev3 = parseResponse(spReview3.content?.[0]?.text || '{}');
    ...
    const projectsData = parseResponse(list2.content?.[0]?.text || '{}').projects || [];
    ...
    const projectsData2 = parseResponse(list3.content?.[0]?.text || '{}').projects || [];
    ...
    const projectsData3 = parseResponse(list4.content?.[0]?.text || '{}').projects || [];
    ...
    const ex0 = parseResponse(ex0Res.content?.[0]?.text || '{}');
    ...
    const ex1 = parseResponse(ex1Res.content?.[0]?.text || '{}');
    ...
    const ex2 = parseResponse(ex2Res.content?.[0]?.text || '{}');
