  ...
  return (
    <ul style={{listStyle:'none',padding:0,margin:0,display:'grid',gap:'0.5rem'}}>
      {Array.from({length:4}).map((_,i)=>(
        <li key={i} style={{border:'1px solid #30363d',borderRadius:6,padding:'0.75rem',background:'#161b22'}}>
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:'0.75rem'}}>
      {['Pending','In Progress','Completed','Archived'].map(col => (
        <div key={col} style={{background:'#161b22',border:'1px solid #30363d',borderRadius:6,padding:'0.5rem',minHeight:300,display:'flex',flexDirection:'column'}}>
          <div style={{fontWeight:600,fontSize:'0.8rem',marginBottom:'0.4rem'}}>{col}</div>
          <div style={{display:'flex',flexDirection:'column',gap:'0.5rem'}}>
            {Array.from({length:4}).map((_,i)=>(
              <div key={i} style={{background:'#0d1117',border:'1px solid #30363d',borderRadius:6,padding:'0.5rem'}}>
