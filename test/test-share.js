...
async function run() {
  ...
  try {
    ...
    const found = (adminList1.projects || []).find(p => p.id === projectId);
    assert(found && found.owner_id === user1.id && found.name === name, 'Admin list should include project with correct owner and name');

    // User2 has no access yet
    const u2List0 = await listProjectsUser(user2.apiKey);
    assert(!(u2List0.projects || []).some(p => p.id === projectId), 'User2 should not see project before share');
    ...
    // User2 sees Read-Only and limited status
    const u2List1 = await listProjectsUser(user2.apiKey);
    const u2FoundRo = (u2List1.projects || []).find(p => p.id === projectId);
    assert(u2FoundRo && u2FoundRo.read_only === true, 'User2 list should show project with read_only flag');
    ...
    // User2 list shows project without read_only flag now
    const u2List2 = await listProjectsUser(user2.apiKey);
    const u2FoundRw = (u2List2.projects || []).find(p => p.id === projectId);
    assert(u2FoundRw && u2FoundRw.read_only !== true, 'User2 list should show project without read_only flag for RW');
    ...
    // Revert to initial
    await client1.callTool({ name: 'revert_project', arguments: { project_id: projectId, hash: initialHash } });
    const readAgent = await client1.callTool({ name: 'read_agent', arguments: { project_id: projectId } });
